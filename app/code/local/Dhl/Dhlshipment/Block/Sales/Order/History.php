 ...
 */
class Dhl_Dhlshipment_Block_Sales_Order_History extends Mage_Core_Block_Template
{

	public function __construct()
	{
		parent::__construct();
		$this->setTemplate('sales/order/history-dhl.phtml');
...
//				var_dump($orders);exit;
	...
	public function getTrackinngAwb($order_id)
	{
		$resource = Mage::getSingleton('core/resource');
		$readConnection = $resource->getConnection('core_read');
		$table = $resource->getTableName('dhlshipment/dhlshipment');
		$query = 'SELECT tracking_awb FROM ' . $table . ' WHERE order_id = '
				. $order_id . ' LIMIT 1';
		$sku = $readConnection->fetchOne($query);
		if($sku==0||$sku=='')
			return '';
		else	
			return '| <a href="?tracking='.$sku.'">Check AWB</a>';
	}
